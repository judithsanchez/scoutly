...
const logger = new Logger('api/internal/verify-user');

export async function POST(request: Request) {
	await logger.debug('POST /internal/verify-user called');
	try {
		...
		if (env.isProd && deployment.isVercel) {
			...
			try {
				await logger.debug('Proxying to backend API', {
					url: `${apiUrl}/internal/verify-user`,
					email,
				});
				const backendRes = await fetch(`${apiUrl}/internal/verify-user`, {
					method: 'POST',
					headers: {
