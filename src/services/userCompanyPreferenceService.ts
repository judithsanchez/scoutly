...
export const UserCompanyPreferenceService = {
	...
	async updateByCompanyId(companyId: string, data: any) {
		...
		if (data?.userId) filter.userId = data.userId;
		if (data?.email) filter.email = data.email;
		const updatedDoc = await UserCompanyPreference.findOneAndUpdate(
			filter,
			data,
			{
				new: true,
		...
		if (updatedDoc) {
			updated = {
				...
				createdAt:
					updatedDoc.createdAt instanceof Date
						? updatedDoc.createdAt.toISOString()
						: String(updatedDoc.createdAt),
				updatedAt:
					updatedDoc.updatedAt instanceof Date
						? updatedDoc.updatedAt.toISOString()
						: String(updatedDoc.updatedAt),
			};
		}
		return {success: true, updated};
	},
